...
// seach 绑定点击事件
var otext = document.getElementsByClassName("showImg"); //获取input框
ose = document.querySelector("#search1"); //通过类名选择器 选择到search框
lis = document.getElementsByClassName("li1"); //获取所有的li
otext.onkeyup = function() { //当在input框中键盘弹起发生事件
    ose.style.display = otext.value ? "block" : "none"; /*三元运算符,如果otext.value的值部位空,则block。*/
    var osc = document.createElement("script"); /*创建一个script标签*/
    osc.src = "http://mall.360.cn/search/?q=" + otext.value + "&cb=zhangjing";
    ...
    if (event.keyCode == 13) {
        ...
        window.location.href = "http://mall.360.cn/search/?q=" + otext.value
    }
}
...
btn.onclick = function() {
    ...
    /*将百度作为连接,传入input的值,并跳入新的页面*/
    window.location.href = "http://mall.360.cn/search/?q=" + otext
}
